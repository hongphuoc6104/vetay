...
for directory in ['sys/skill','sys/templates','sys/engine']:
 for p in (root/directory).rglob('*'):
  if p.is_file() and not any(x in ['node_modules','__pycache__','.venv','cache','work','logs','models','video'] for x in p.relative_to(root).parts) and p.suffix.lower() not in ['.pyc','.wav','.mp3','.mp4','.webm','.log']:files.append(p)
