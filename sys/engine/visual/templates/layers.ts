...
export class LayerStack{
 ...
 constructor(colors:string[],ink:string){this.renderer=new THREE.WebGLRenderer({alpha:true,antialias:true});this.renderer.setSize(1080,1920);this.renderer.setClearColor(0,0);this.camera.position.z=7;
 ...
 colors.forEach((color,i)=>{const canvas=document.createElement('canvas');canvas.width=800;canvas.height=400;const c=canvas.getContext('2d')!;c.fillStyle=color;c.fillRect(0,0,800,400);c.strokeStyle=ink;c.fillStyle=ink;c.lineWidth=8;c.lineCap='round';
