...
export function drawTemplate(ctx:CanvasRenderingContext2D,e:any,t:number,theme:string,env:any){
 ...
 if(s.id==='focus'){
  ...
  for(const d of s.drawings||[]){
   ...
   const total=paths.reduce((n:number,p:number[][])=>n+p.length,0),progress=fade(d.cue,d.seconds||1.8);let consumed=0;
   for(const points of paths){const [x,y,w,h]=d.box,mapped=points.map((p:number[])=>[x+p[0]*w/100,y+p[1]*h/100]);const partial=clamp((progress*total-consumed)/points.length);consumed+=points.length;if(d.fill&&partial>=1){ctx.save();ctx.globalAlpha*=.09;ctx.fillStyle=C[d.fill];ctx.beginPath();mapped.forEach((p:number[],i:number)=>i?ctx.lineTo(p[0],p[1]):ctx.moveTo(p[0],p[1]));ctx.fill();ctx.restore();}strokes.push({points:mapped,progress:partial,color:C[d.color||'teal'],width:d.lineWidth||6});}
  }
 ...
 for(const a of s.annotations||[]){const progress=fade(a.cue,a.seconds||.65);if(progress<=0)continue;const target=boxes.filter(b=>b.id===a.targetId);if(!target.length)throw Error('Annotation target not visible: '+a.targetId);
  ...
  for(const b of target){let points;if(a.kind==='circle'){const pad=24;points=Array.from({length:121},(_,i)=>{const n=i/120*Math.PI*2;return [b.x+b.w/2+(b.w/2+pad)*Math.cos(n),b.y+b.h/2+(b.h/2+pad)*Math.sin(n)];});}else{const y=a.kind==='strike'?b.y+b.h*.5:b.y+b.h+20;points=[[b.x,y],[b.x+b.w*.5,y+2],[b.x+b.w,y]];}strokes.push({points,progress,color:C[a.color||'gold'],except:a.kind==='strike'?a.targetId:undefined});}
 }
 for(const stroke of strokes){const pts=partialPath(stroke.points,stroke.progress);if(pts.length<2)continue;if(pts.some((p:any)=>p[0]<86||p[0]>994||p[1]<546||p[1]>1644))throw Error('Drawing leaves safe workspace');const hit=collision(pts,boxes,Math.max(6,(stroke.width||6)/2),stroke.except);if(hit)throw Error('Pen overlaps protected text: '+hit);P.stroke(ctx,pts,1,stroke.color,stroke.width||6,false);if(stroke.progress<1){const tip=pts.at(-1)!;ctx.fillStyle=C.gold;ctx.beginPath();ctx.arc(tip[0],tip[1],5,0,Math.PI*2);ctx.fill();}}
 // Header/caption are outside the template workspace; ensure text cannot intrude.
 for(const b of boxes)if(b.y<540||b.y+b.h>1650||b.x<80||b.x+b.w>1000)throw Error('Template content leaves safe area: '+b.id);
 diagnostics.template={id:s.id,boxes,strokeCount:strokes.length,time:t};
}
