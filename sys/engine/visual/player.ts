...
function paint(ctx:CanvasRenderingContext2D,t:number){
 ...
 if(s.logo&&!(index===scenes.length-1&&publication.outro?.enabled===true)){P.alpha(ctx,smooth(t,s.end-1.7,s.end-1.2),()=>{ctx.save();ctx.beginPath();ctx.arc(540,1450,90,0,7);ctx.clip();ctx.drawImage(logos[s.theme],450,1360,180,180);ctx.restore();});}
 drawOutro(ctx,s,s.theme,index,t);
