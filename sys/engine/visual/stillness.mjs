...
import {workspace,workspaceCrop} from './workspace.mjs';
/** Compare to a stable anchor, not only adjacent frames: slow pen strokes accumulate. */
export async function contentHolds(file,layout){
 const width=460,height=Math.round(workspace(layout).height/2),size=width*height;const p=spawn('ffmpeg',['-v','error','-i',file,'-vf',`${workspaceCrop(layout)},fps=2,scale=${width}:${height}`,'-pix_fmt','gray','-f','rawvideo','pipe:1']);let buffer=Buffer.alloc(0),anchor,lastActivity=0,n=0,stderr='';const holds=[];
