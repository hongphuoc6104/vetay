...
try{
 const scenes=validateProject(project,timeline,brand);const publication=publicationMeta(project,timeline);const warnings=publicationWarnings(project,timeline);for(const warning of warnings)console.warn('PUBLICATION:',warning);const start=Number(get('--start',0)),end=Number(get('--end',timeline.targetSeconds));if(start<0||end>timeline.targetSeconds+1/30||end<=start)throw Error('Invalid preview range');
 ...
 const coverFilter=`select=eq(n\\,${coverLocalFrame})`;await run('ffmpeg',['-y','-v','error','-i',mp4,'-vf',coverFilter,'-frames:v','1','-vsync','vfr',path.join(output,name+'-cover.png')]);
