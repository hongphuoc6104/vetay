...
try{
 ...
 if(transport==='legacy-png')await fs.copyFile(framePaths[Math.floor(framePaths.length*.8)],path.join(output,name+'-cover.png'));else await run('ffmpeg',['-y','-v','error','-ss',String((last-first)/fps*.8),'-i',mp4,'-frames:v','1',path.join(output,name+'-cover.png')]);
